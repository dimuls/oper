...
type Server struct {
	...
	stop         chan struct{}
	wg           sync.WaitGroup
	service.UnimplementedPointerServer
}
...
func NewServer(s Storage) (*Server, error) {
	...
	service.RegisterPointerServer(srv.grpcServer, srv)
	...
	kafkaClient.AssignGroup(service.PointerServiceKafkaGroupID,
		kgo.GroupTopics(service.PointerServiceName))
...
func (s *Server) AccountAmount(ctx context.Context, req *service.PointsAmountReq) (
	*service.PointsAmountRes, error) {
	...
	return &service.PointsAmountRes{
		Amount: amount,
	}, nil
}
